...
family_list = ['L1P_5end', 'MER11', 'LTR5']

s = time.time()
print('start searching binding sites of TE')
...
# map files
map_dict = dict()
for i, family in enumerate(pd.unique(Dfam_RM_family['repeat family name'])):
    ...
    try:
        ...
        #print(i, family, time.time()-s, 'ERROR')
        pass
...
# obtain KZFP peak data
Dfam_KZFP_familly_dict = dict()
for i, (KZFP, target_list) in enumerate(target_dict.items()):
...
for i, (KZFP, exp) in enumerate(KZFP_ChIP_metadata[['KZFP gene symbol', 'KZFP experiment']].value_counts().index):
...
motif_metadata_modified_df.to_csv('../data/motif/TE_KZFP_binding_sites.csv', index=False)

print(time.time()-s, 'finish searching binding sites of TE')
